...
def calculate_actions_per_minute(actions_count, start_time_local):  # Calculates Actions or Effective Actions Per Minute
    elapsed_time = max(time.time() - start_time_local, 0.001)  # prevent division by zero
...
def update_display():
    ...
    current_APM = round(calculate_actions_per_minute(keystrokes + mouse_clicks, start_time))
    APM_list.append(current_APM)

    current_EAPM = round(calculate_actions_per_minute(effective_actions, start_time))
    EAPM_list.append(current_EAPM)

    avg_APM = round(calculate_actions_per_minute(sum(APM_list) / len(APM_list), start_time))
    peak_APM = max(APM_list)

    avg_EAPM = round(calculate_actions_per_minute(sum(EAPM_list) / len(EAPM_list), start_time))
    peak_EAPM = max(EAPM_list)

    # Update GUI
    apm_label.config(text="Current APM: {}".format(current_APM))
    avg_apm_label.config(text="Average APM: {}".format(avg_APM))
    peak_apm_label.config(text="Peak APM: {}".format(peak_APM))

    effective_APM_label.config(text="Current EAPM: {}".format(current_EAPM))
    avg_effective_APM_label.config(text="Average EAPM: {}".format(avg_EAPM))
...
def start_gui():
    global root, apm_label, avg_apm_label, peak_apm_label, effective_APM_label, avg_effective_APM_label, peak_eapm_label, keyboard_listener, mouse_listener, canvas, frame_graph
    ...
    avg_apm_label = tk.Label(root, text="Average APM: 0", font=font_style_regular)
    avg_apm_label.grid(row=1, column=0, padx=20, pady=10)
    ...
    avg_effective_APM_label = tk.Label(root, text="Average EAPM: 0", font=font_style_regular)
    avg_effective_APM_label.grid(row=1, column=1, padx=20, pady=10)
